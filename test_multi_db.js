// 多数据库功能测试脚本
import { testAllConnections, getAllDatabaseStats, savePosts, isGuidExists, closeAllConnections } from './src/db.js';

async function testMultiDatabase() {
  console.log('🚀 开始多数据库功能测试...\n');
  ...
  try {
    ...
    const testPosts = [
      {
        title: '测试帖子标题 - 多数据库测试',
        creator: 'test_user',
        description: '这是一个多数据库功能测试帖子',
        link: 'https://linux.do/t/topic/test-123',
        pubDate: new Date().toISOString(),
        guid: `test-multi-db-${Date.now()}`,
    ...
    statsBefore.forEach((beforeStat, index) => {
      ...
      if (beforeStat.status === 'healthy' && afterStat.status === 'healthy') {
        ...
        console.log(`${beforeStat.name}: ${beforeStat.totalPosts} → ${afterStat.totalPosts} (+${increase})`);
      }
    });
